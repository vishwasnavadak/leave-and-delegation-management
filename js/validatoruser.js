...
function validate(){	
	...
	if(!date_vaild.test(startdate)){
		document.getElementById("er1").innerHTML="Invalid Date. Enter valid date in DD-MM-YY format only.<br/>";
		return false;
	}
	...
	var nsd=startdate.split("-"); // split the string data received to array
	var ned=enddate.split("-");
	var f1=datevalid(20+nsd[2],nsd[1],nsd[0]);
	var f2=datevalid(20+ned[2],ned[1],ned[0]);
	var stdate=new Date(20+nsd[2],nsd[1]-1,nsd[0]); //create date object using the array
	...
	if(f1===0 || f2===0){
		...
		document.getElementById("er1").innerHTML = "Start Date must be smaller than end date<br/>";
		return false;
	}
	else
		return true;
